...
export default function HospitalOrder({ hospital, user, item, placeOrder }) {
    ...
    async function handleOrder() {
        ...
        if (localAmount - inputRef.current.value > 0) {
            axios
                .patch(
                    `http://localhost:6789/api/replenishStock/${item._id}`,
                    {
                        amount: localAmount - inputRef.current.value,
                    },
                    {
                        headers: { "Content-type": "application/json" },
                    }
                )
            setLocalAmount(localAmount-inputRef.current.value)
            placeOrder(localAmount)
            ...
            let data = {
                ...
                "bottom-notice": "Kindly pay your invoice within 15 days otherwise our people will find you.",
    ...
    return (
        <>
            <div className='hospital'>
                ...
                <input ref={inputRef} type="number" value={inputValue} onChange={handleInputChange} className='order-number'></input>
